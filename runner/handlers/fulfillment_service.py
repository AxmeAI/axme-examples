...
def handle(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if shipping not in _SHIPPING_DAYS:
        violations.append(
            f"shipping_tier_unknown:{shipping!r} "
            f"(allowed: {', '.join(sorted(_SHIPPING_DAYS))})"
        )
